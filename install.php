...
if( $wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name ) {
	...
	*/
	...
	add_option( "wpstp_db_version", $wpstp_db_version );
	add_option( "wpstp_version",  $wpstp_version );
	add_option( "wpstp_max_rows_per_spider", 30 ); 
	add_option( "wpstp_slide_delay", 300 );
	add_option( "wpstp_pct_bar_color", "red" );
	add_option( "wpstp_log_state", 'c' );
}

// Upgrade
$installed_ver = get_option( "wpstp_db_version" );
if( $installed_ver != $wpstp_db_version ) {
	$sql = "CREATE TABLE IF NOT EXISTS `". $table_name ."` (
			`id` DOUBLE NOT NULL AUTO_INCREMENT ,
			`name` VARCHAR( 120 ) NOT NULL ,
			`useragent` VARCHAR( 225 ) NOT NULL ,
	...
	update_option( "wpstp_db_version", $wpstp_db_version );
}
?>